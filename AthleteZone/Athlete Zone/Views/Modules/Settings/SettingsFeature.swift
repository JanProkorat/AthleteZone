...
struct SettingsFeature {
    ...
    var body: some ReducerOf<Self> {
        Reduce { state, action in
            switch action {
            case .onAppear:
                state.healthKitAccess = healthManager.getHealthAccessGranted()
                return .run { send in
                    ...
                    await send(.hkAccessStatusChanged)
                }
            ...
            case .hkAccessStatusChanged:
                state.hkAuthStatus = healthManager.checkAuthorizationStatus()
                return .none

            case .healthKitAccessChanged(let enabled):
                state.healthKitAccess = enabled
                if !enabled {
                    return .none
                }
                var status = healthManager.checkAuthorizationStatus()
                if status == .sharingAuthorized {
                    state.healthKitAccess = true
                    return .none
                }
                healthManager.requestAuthorization()
                status = healthManager.checkAuthorizationStatus()
                if status != .sharingAuthorized {
                    state.healthKitAccess = false
                }
                state.hkAuthStatus = status
                return .none
